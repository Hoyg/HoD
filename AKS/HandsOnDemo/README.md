...
Azure Kubernetes Service brings so many amazing features for developers. The container health dashboard, deep log search features enabling you to really see how your code's executing in the cluster, and IDE/debugger integration that makes it possible for you to edit and debug code live in the cluster without impacting production or teammates' code - making AKS the greatest experience for building apps with Kubernetes.

## Content

In this demo, the presenter is a new developer working for a hotel company - SmartHotel360, who has been assigned a bug to fix in a huge AKS cluster with a variety of microservices written in Node.js, .NET Core, and Java. The developer is armed with a few pre-written queries to see logs, CPU usage, and a short description of a defect in the company's web site.
...
1. Search for **New** (note the capitalization) using the city search feature on the public web site. 
    ...
    > **Talk track:** I'm a new developer who has been assigned to work on a bug. I work for a hotel company, and our customers can search for a few cities, but we've noticed that most cities aren't showing up any more. The phrase `New` returns some matches.
...
1. Search for **Sea** or **Seattle** to demonstrate that no results are found. 
    ...
    > **Talk track:** But when I search for `Seattle`, I don't see anything, and I know we have hotels in Seattle. I've been assigned this bug, and given a few pointers on where to get the code and that it is running in **Azure Kubernetes Service**. 

1. Navigate to the AKS overview page in the Azure Portal to see the demo cluster. 

    ![The AKS Cluster in the portal](../media/demo-script/03-cluster.png)

    > **Talk track:** I'll flip over to the Azure portal to take a look at this AKS cluster in more detail. Maybe I can find something in the portal that'll help me figure out how to fix the bug. The **AKS Overview** page gives me a quick snapshot of how the cluster's operating, but I'll need to go a little deeper. Maybe the **Health** link will help. 
...
1. Wait for the default query to run. If no results are shown in the results pane, increase the parameter for the `ago` method to a greater duration - `ago(24)`, for the past 24 hours of log data. 
...
1. Expand one of the "*City search returned 0 results*" log entries. 
    ...
    > **Talk track:** These log entries are a good clue in providing evidence of where the bug's happening, but I can get even deeper. Other log entries in the view show me which ASP.NET MVC Controller are dropping in those "0 results" log entries. 

1. Expand one of the `CitiesController` log entries. 

    ![](../media/demo-script/10-logs-help-the-final-view.png)

    > **Talk track:** I can see from the Log Search that the exact class performing the logging is the `CitiesController` class. Now I know the right place to look in the source code. Finding the source of this bug only took me seconds. It seems like a good time to procrastinate, so let me use some of the saved queries I have here to look around a little. 

1. Expand the **Saved Queries** under the **Query Explorer** window in the Azure portal and click the **CPU over time** query. 

    ![CitiesController](../media/demo-script/10-logs-help-cpu-over-time.png)

    > **Talk track:** I'll execute the **CPU over time** query.
...
1. Click on the **Logs** query and click the **Run** button.
...
1. Click on the **Logs Chart** query and click the **Run** button.
...
1. Flip over to the Visual Studio editor where the solution is open. 
    ...
    > That's a lot of code, but luckily I can use Visual Studio's world-class productivity features like code navigation and type search to find what I'm looking for in this huge solution - the `CitiesController` class. 

1. Hit `Ctrl-T` to open the Visual Studio `Go to all` search pane.

1. Type `CitiesController`, find the `CitiesController` class and click on it. 
    
    ![Searching for a type](../media/demo-script/12-vs-ctrl-t-to-search.png)

    > **Talk track:** Here's the CitiesController class. Now I can take a look at the `Get` method to see if I can tell what's wrong. Looks like the previous developer is making two calls to the `GetDefaultCities` method here. I'll use **Go to Definition** to see what that method is doing. 
...
1. Scroll up to show the `Get` method. 
    ...
    > **Talk track:** Looks like the `GetDefaultCities` method returns a static list. The list doesn't contain Seattle, so that explains why it isn't showing up. If I look up this method, I see that the `Get` method is the one that actually uses Entity Framework to query the database. 
...
1. Change the code to match this:
    ...
    > **Talk track:** Now that I have the code edited, I'd like to debug it to make sure it works. I'll set my breakpoint right here on the line of code in the controller when the results are returned. 
...
1. Right-click the `SmartHotel.Services.Hotels` Visual Studio project and select **Properties** from the flyout menu. 

    > **Talk track:** Luckily, that's exactly what **Azure Dev Spaces** can do for me. Without setting up anything, I can select Dev Spaces to use when I want to debug my code. Then, a copy of my service is deployed into AKS, and commands will be routed to that copy of the service. That way, I can debug my code changes **in my own space** without impacting either the production environment **or** my teammates, all of whom may have code running in their own Dev Spaces. 

1. Select **Debug** in the left navigation and Select **Azure Dev Spaces** from the **Profile** menu. 

    ![Selecting Dev Spaces](../media/demo-script/17-select-dev-spaces.png)

    > **Talk track:** Here I'll specify that I want to use Azure Dev Spaces as my debugging target. Next, I'll create my own space for the code to run during my debugging session. 

1. Click the **Change** button and select the AKS Cluster you want to use. Select **Create New Space** from the Space drop down. 

    ![Create a new Space](../media/demo-script/18-create-new-space.png)

    > **Talk track:** It's as easy as creating a new space in which my code will run. I'll do that here. When I hit **F5**, my code will run in AKS in my own Dev Space. 
...
1. Enable the **Launch browser** checkbox and paste the URL of the site (this should be the web site's public URL running in your cluster).
...
1. Once the site opens, zoom in on the URL to show the Dev Space prefix. Scroll down and search for **Seattle** in the city search box. In a moment, Visual Studio should obtain focus and the debugger should stop on the line with the breakpoint. 
    
    ![Search for Seattle](../media/demo-script/24-breakpoint-hit.png)

    > **Talk track:** Now, when I search, the HTTP call will be sent to an instance of the Hotels service that I'm actively debugging. I can see the code running in the debugger, and all the watches, locals show up. I can see right away that the search hit the database and found a result for "Seattle." 

    > I can hit F5 again to let the code continue running. 
    
    ![Success](../media/demo-script/25-search-works.png)

    > **Talk track:** Now, the search is working. I'm ready to commit my code. *I was able to find and fix my bug in minutes.* 

## Summary

Azure Kubernetes Service lets you get the best end-to-end experience for building apps in containers using Kubernetes. With the Container Health Monitoring, you will have deep telemetry on how the code is running in your AKS cluster.  When issues arise, you will use the integrated Azure Dev Spaces in Visual Studio and VS Code to build and debug your applications in your own spaces, without messing with teammates or production code, and without having to spend days getting up and running.
